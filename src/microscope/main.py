...
def main(filename: str, show_dag: bool, qiskit_fallback: bool):
    ...
    qc_dag = circuit_to_dag(transpiled_qc)
    print(f"Number of swaps in Qiskit SABRE: {len(qc_dag.op_nodes(op=SwapGate))}")
    ...
    swaps = []
    for i, node in transpiled_sabre_dag.nodes.items():
        if node.is_swap:
            swaps.append(node)

    print(f"Number of swaps in MicroSABRE: {len(swaps)}")
    ...
    print(
        f"Number of swaps in merged MicroSABRE: {len(transpiled_qiskit_sabre_dag.op_nodes(op=SwapGate))}"
    )
